...
export function LampDemo() {
  ...
  const handleCardClick = (testId: number) => {
    ...
    if (isAuthorized) {
      ...
      router.push(`/sign-in?testId=${testId}`);
    }
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#04314D] overflow-hidden">
      <div className="flex w-full flex-1 scale-y-100 items-center justify-center isolate z-0 pt-16">
        ...
        >
          <div className="absolute w-[100%] left-0 bg-[#04314D] h-40 bottom-0 z-20 [mask-image:linear-gradient(to_top,white,transparent)]" />
          <div className="absolute w-40 h-[100%] left-0 bg-[#04314D] bottom-0 z-20 [mask-image:linear-gradient(to_right,white,transparent)]" />
        </motion.div>
        ...
        >
          <div className="absolute w-40 h-[100%] right-0 bg-[#04314D] bottom-0 z-20 [mask-image:linear-gradient(to_left,white,transparent)]" />
          <div className="absolute w-[100%] right-0 bg-[#04314D] h-40 bottom-0 z-20 [mask-image:linear-gradient(to_top,white,transparent)]" />
        </motion.div>

        <div className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-[#04314D] blur-2xl"></div>
        <div className="absolute top-1/2 z-50 h-48 w-full bg-transparent opacity-10 backdrop-blur-md"></div>
        <div className="absolute inset-auto z-50 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-[#75D8FF] opacity-80 blur-3xl"></div>
      ...
      <div className="relative z-50 flex flex-col items-center px-5 -translate-y-10">
        <motion.h1
          ...
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          className="mt-8 bg-gradient-to-br from-[#B6E8FF] to-[#009FE2] py-4 bg-clip-text text-center text-4xl font-medium tracking-tight text-transparent md:text-7xl"
        >
          Личный кабинет
        </motion.h1>
      </div>
      ...
      <div className="relative z-40 px-4 pb-12 mt-4">
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#009FE2]"></div>
          </div>
        ) : error ? (
